        ...
        <section className="ge-blog-page__content">
          <div className="ge-container">
            ...
            <article className="ge-blog-featured">
              <Link href={`/${featured.slug}/`} className="ge-blog-featured__visual" aria-label={`Read ${featured.title}`}>
                {featured.image ? <Image src={featured.image} alt={featured.imageAlt ?? ""} fill sizes="(max-width: 900px) 100vw, 52vw" priority /> : <><span>SEO</span><strong>Search<br />visibility<br />starts here.</strong><i>01</i></>}
              </Link>
              <div className="ge-blog-featured__content">
                <p className="ge-eyebrow">{featured.category} <span /> {formatBlogDate(featured.date)}</p>
                <h2><Link href={`/${featured.slug}/`}>{featured.title}</Link></h2>
                <p>{featured.excerpt}</p>
                <Link className="ge-text-link" href={`/${featured.slug}/`}>Read article <span aria-hidden="true">↗</span></Link>
              </div>
            </article>
            {posts.length > 0 && (
